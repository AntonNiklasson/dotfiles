...
const data = JSON.parse((await $`gh pr view ${prUrl} --json title,additions,deletions,baseRefName`).stdout)
const { title, additions, deletions, baseRefName } = data
...
let targetInfo = ''
if (baseRefName !== 'main') {
  const targetingPrs = JSON.parse(
    (await $`gh pr list --base ${baseRefName} --author @me --state open --json url,number`).stdout
  )
  if (targetingPrs.length > 0) {
    targetInfo = ` → ${baseRefName} (#${targetingPrs[0].number})`
  } else {
    targetInfo = ` → ${baseRefName}`
  }
}

const message = `[${cleanTitle}](${prUrl}) \`+${additions}/-${deletions}\`${targetInfo}`

await $`echo -n ${message} | pbcopy`
console.log('\x1b[32m✓\x1b[0m copied')
console.log(message)
